...
public class FillerExactCookingPredicate extends CookingPredicate {
    public static final MapCodec<FillerExactCookingPredicate> CODEC = RecordCodecBuilder.mapCodec(
        i -> i.group(
            IngredientCategoryKey.CODEC
                .fieldOf("category")
                .forGetter(p -> p.category),
            Codec.FLOAT.fieldOf("value").forGetter(p -> p.value)
        ).apply(i, FillerExactCookingPredicate::new));

    private final IngredientCategoryKey category;
    private final float value;

    public FillerExactCookingPredicate(
