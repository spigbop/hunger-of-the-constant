...
public class FillerLessThanCookingPredicate extends CookingPredicate {
    public static final MapCodec<FillerLessThanCookingPredicate> CODEC = RecordCodecBuilder.mapCodec(
        i -> i.group(
            IngredientCategoryKey.CODEC
                .fieldOf("category")
                .forGetter(p -> p.category),
            Codec.FLOAT.fieldOf("value").forGetter(p -> p.value)
        ).apply(i, FillerLessThanCookingPredicate::new));

    private final IngredientCategoryKey category;
    private final float value;

    public FillerLessThanCookingPredicate(
