...
public class FillerMoreThanCookingPredicate extends CookingPredicate {
    public static final MapCodec<FillerMoreThanCookingPredicate> CODEC = RecordCodecBuilder.mapCodec(
        i -> i.group(
            IngredientCategoryKey.CODEC
                .fieldOf("category")
                .forGetter(p -> p.category),
            Codec.FLOAT.fieldOf("value").forGetter(p -> p.value)
        ).apply(i, FillerMoreThanCookingPredicate::new));

    private final IngredientCategoryKey category;
    private final float value;

    public FillerMoreThanCookingPredicate(
